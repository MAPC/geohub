...
A tool that turns any town-level spreadsheet into an interactive, color-coded map (a "choropleth") of the 101 cities and towns that make up MAPC's Greater Boston region. Everything lives in one file — `mapc-choropleth-map.html` — no installation or server required.

## What the HTML file does

Open `mapc-choropleth-map.html` in any browser (just double-click it) and it:
...
- Pick a color ramp (including MAPC's own blue and green) or switch the basemap style (Light, Voyager, Dark, or OpenStreetMap).
